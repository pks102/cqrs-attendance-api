...
public class EodAttendanceController {
    private final Logger logger = Logger.getLogger(EodAttendanceController.class.getName());

    @Autowired
    private CommandDispatcher commandDispatcher;

    @PutMapping("/v1/eodAttendance")
    public ResponseEntity<BaseResponse> calculateEodAttendance(@RequestBody EodAttendanceCommand command) {
        try {
            commandDispatcher.send(command);
            return new ResponseEntity<>(new BaseResponse(command.getEmpId() + " employee hours calculated"), HttpStatus.CREATED);
